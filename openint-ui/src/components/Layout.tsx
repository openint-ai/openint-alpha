...
const PRODUCT_FEATURES = [
  { path: '/', label: 'Vector DB Chat Demo' },
...
export default function Layout() {
  ...
  const currentFeature = PRODUCT_FEATURES.find((f) => f.path === loc.pathname || (f.path === '/' && (loc.pathname === '/' || loc.pathname === '/chat')));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            <div className="relative" ref={featuresRef}>
              <button
                ...
                className={`inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  currentFeature ? 'bg-brand-100 text-brand-700' : 'text-gray-600 hover:text-gray-900 hover:bg-surface-100'
                }`}
                aria-expanded={featuresOpen}
                aria-haspopup="true"
                aria-label="Product Features menu"
              >
                Product Features
              ...
              {featuresOpen && (
                <div className="absolute left-0 top-full mt-1 min-w-[220px] rounded-xl border border-surface-200 bg-white py-1 shadow-lg z-40">
                  {PRODUCT_FEATURES.map(({ path, label }) => (
                    <Link
                      key={path}
                      to={path}
                      className={`block px-4 py-2.5 text-sm font-medium transition-colors ${
                        currentFeature?.path === path ? 'bg-brand-50 text-brand-700' : 'text-gray-700 hover:bg-surface-50'
                      }`}
                    >
                      {label}
                    </Link>
      ...
      <main className={`flex-1 w-full mx-auto py-4 sm:py-6 md:py-8 ${loc.pathname === '/compare' || loc.pathname === '/a2a' || loc.pathname === '/graph' ? 'max-w-full px-2 sm:px-4' : loc.pathname === '/' || loc.pathname === '/chat' ? 'max-w-full px-2 sm:px-4 md:px-6' : 'max-w-4xl px-2 sm:px-4 md:px-6'}`}>
        <Outlet />
      </main>
    </div>
  );
