# Sentence Augmentation Agent (sa-agent)

The **Sentence Augmentation Agent (sa-agent)** connects to DataHub (running locally), reads dataset schemas, and uses a **fine-tuned Llama 3 model** (Ollama) to **enrich and annotate** sentences while **preserving original intent**. Produces example sentences that:
...
2. **Sentence generation (Ollama, open-source LLM)**
   ...
   - Requires Ollama running locally. Model: `OLLAMA_MODEL` (default `qwen2.5:7b`). Run `ollama serve` and `ollama pull qwen2.5:7b` (or another model).
...
| `OLLAMA_MODEL` | Ollama model name for sentence generation | `qwen2.5:7b` |
...
- **Sentence augmentation:** Ollama running locally with fine-tuned Llama 3 (e.g. `ollama serve` and `ollama pull llama3`). No external API key required.
